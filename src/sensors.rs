...
use esp_hal::{i2c::master::{Config, I2c, }, peripherals::{GPIO8, GPIO9, I2C0}, Blocking};
...
impl BH1750 {
    async fn init_sensor(&self, i2c_bus: &mut I2c<'_, Blocking>) -> Result<(), SensorError> {
        // start-up sequence
...
impl BME280Builder {
    async fn init_sensor(self, i2c_bus: &mut I2c<'_, Blocking>) -> Result<BME280, SensorError> {
        // start-up sequence
        let init_commands = [
...
pub async fn sensor_task(
    ...
    loop {
        if let Ok(reading) = light_sensor.read_sensor(&mut i2c).await {
            SENSOR_CHANNEL.send(reading).await;
            defmt::info!("Sent light reading");
        }
